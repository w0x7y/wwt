//! The row and column diacritics unicode placeholders are addressed with.
//!
//! Vendored from kitty's `gen/rowcolumn-diacritics.txt`, which is the
//! protocol's own list. It is data rather than arithmetic: the codepoints
//! are combining marks of class 230 that carry no decomposition mapping, so
//! a terminal will not fuse or normalise them away, and nothing about the
//! sequence can be computed from an index.

pub const CODES: [char; 297] = [
...
mod tests {
    ...
    fn the_table_starts_where_the_protocol_says_it_does() {
        // U+0305 COMBINING OVERLINE. If this fails, the vendored file is not
        // the one the protocol means.
