# wwt M2 — Navigation and Reading Implementation Plan
...
**Spec:** `docs/superpowers/specs/2026-08-19-wwt-m2-design.md` — read it in full before starting. Its parent, `docs/superpowers/specs/2026-08-19-wwt-design.md`, governs where the two disagree; sections 5, 6, and 8 of the parent are the relevant ones.
...
- `wwt-frame` has **no I/O and no dependencies**. This is unchanged and non-negotiable.
...
Expected: 46 tests pass — 17 `wwt-frame`, 12 `wwt-term`, 3 `wwt-cdp`, 12 `wwt-page`, 2 `wwt`.

---

### Task 1: `Style::reverse` and `Frame::paint_text`

The chrome row needs to paint text at an exact cell and needs reverse video. Both belong in `wwt-frame`, which stays dependency-free. Adding a field to `Style` breaks every struct literal in the workspace, so this task fixes them all.

**Files:**
- Modify: `crates/wwt-frame/src/cell.rs:10-22` (add `reverse`, update `Default`)
- Modify: `crates/wwt-frame/src/frame.rs` (add `paint_text`, add tests)
- Modify: `crates/wwt-frame/src/frame.rs:273` (existing `Style` literal)
- Modify: `crates/wwt-term/src/render.rs:104,111,119` (existing `Style` literals in tests)
- Modify: `crates/wwt-page/src/extract.rs:175-178` (existing `Style` literal)
...
Add to the `mod tests` block at the bottom of `crates/wwt-frame/src/frame.rs`:
...
In `crates/wwt-frame/src/cell.rs`, replace the `Style` struct and its `Default`:
...
In `crates/wwt-frame/src/frame.rs`, add to `impl Frame`, directly after `paint_run`:
...
`crates/wwt-frame/src/frame.rs:273`:

```rust
        r.style = Style { fg: Rgb { r: 255, g: 0, b: 0 }, bold: true, reverse: false };
```

`crates/wwt-term/src/render.rs`, lines 104, 111, 119 respectively:
...
`crates/wwt-page/src/extract.rs:175-178`:
...
```bash
git add crates/wwt-frame crates/wwt-term crates/wwt-page
...
**Files:**
- Modify: `crates/wwt-term/src/render.rs` (add `Renderer`, teach `write_style` about `reverse`)
- Modify: `crates/wwt-term/src/lib.rs` (export `Renderer`)

**Interfaces:**
- Consumes: `Style.reverse` from Task 1.
- Produces: `wwt_term::Renderer` with `Renderer::new()`, `Renderer::render(&mut self, frame: &Frame, out: &mut impl Write) -> std::io::Result<()>`, `Renderer::invalidate(&mut self)`. The free `wwt_term::render` is unchanged in signature.

- [ ] **Step 1: Write the failing tests**

Add to `mod tests` in `crates/wwt-term/src/render.rs`:
...
In `crates/wwt-term/src/render.rs`, replace `write_style`:
...
Add to `crates/wwt-term/src/render.rs`, above the `mod tests` block:
...
**Files:**
- Modify: `crates/wwt-cdp/src/client.rs` (add `Event`, `subscribe`, broadcast in `read_loop`, tests)
- Modify: `crates/wwt-cdp/src/lib.rs` (export `Event`)

**Interfaces:**
- Consumes: nothing.
- Produces: `wwt_cdp::Event { session_id: Option<String>, method: String, params: serde_json::Value }` deriving `Debug, Clone`; `Client::subscribe(&self) -> tokio::sync::mpsc::UnboundedReceiver<Event>` (synchronous, not `async`).

- [ ] **Step 1: Write the failing tests**

Add a `mod tests` block at the bottom of `crates/wwt-cdp/src/client.rs`:
...
In `crates/wwt-cdp/src/client.rs`, add to the imports:
...
Replace `read_loop` in `crates/wwt-cdp/src/client.rs`:
...
**Files:**
- Modify: `crates/wwt-page/src/extract.rs:35-46` (struct and `open`)
- Modify: `crates/wwt-page/tests/extraction.rs:18-36` (harness)
- Modify: `crates/wwt/src/lib.rs:15-17`

**Interfaces:**
- Consumes: `wwt_cdp::Client` from Task 3.
- Produces: `wwt_page::Page` (no lifetime parameter); `Page::open(client: Arc<Client>, url: &str, vp: Viewport) -> Result<Page>`; `Page::session_id(&self) -> &str`.

- [ ] **Step 1: Change the struct**

In `crates/wwt-page/src/extract.rs`, add `use std::sync::Arc;` to the imports, then replace lines 35-46:
...
In `crates/wwt-page/tests/extraction.rs`, add `use std::sync::Arc;` and replace lines 18-36. The lifetime comment on `Harness` goes away with the lifetime:
...
In `crates/wwt/src/lib.rs`, add `use std::sync::Arc;` and replace lines 15-17:
...
```bash
git add crates/wwt-page crates/wwt
...
**Files:**
- Rename: `crates/wwt-page/assets/extract.js` → `crates/wwt-page/assets/bootstrap.js`
- Modify: `crates/wwt-page/assets/bootstrap.js` (wrap the body in `window.__wwt.extract`)
- Modify: `crates/wwt-page/src/extract.rs` (install on open, call the function)

**Interfaces:**
- Consumes: `Page` from Task 4.
- Produces: the page-side global `window.__wwt.extract()` returning `{ runs, title, url, scrollY, scrollHeight, innerHeight }`. `Page::extract` still returns `Result<Vec<TextRun>>` at the end of this task; Task 6 changes that.
...
```bash
git mv crates/wwt-page/assets/extract.js crates/wwt-page/assets/bootstrap.js
```

`crates/wwt-page/assets/bootstrap.js`:
...
In `crates/wwt-page/src/extract.rs`, change the constant:
...
**Files:**
- Modify: `crates/wwt-page/assets/bootstrap.js` (listeners)
- Modify: `crates/wwt-page/src/extract.rs` (`Runtime.enable`, `addBinding`, `Extraction` type)
- Modify: `crates/wwt-page/tests/extraction.rs` (call sites, new test)
- Create: `crates/wwt-page/tests/fixtures/mutating.html`
- Modify: `crates/wwt/src/lib.rs:19` (call site)

**Interfaces:**
- Consumes: `wwt_cdp::Event` from Task 3; `Page` from Task 5.
- Produces: `wwt_page::Extraction { runs: Vec<TextRun>, title: String, url: String, scroll_y: f64, scroll_height: f64, viewport_height: f64 }` with `Extraction::scroll_progress(&self) -> f64`; `Page::extract(&self) -> Result<Extraction>`; `pub const DIRTY_BINDING: &str = "__wwt_dirty"`.

- [ ] **Step 1: Write the failing tests**

Add to `crates/wwt-page/tests/extraction.rs`:
...
`crates/wwt-page/tests/fixtures/mutating.html`:
...
Expected: FAIL — `cannot find value DIRTY_BINDING in crate wwt_page`, `cannot find struct Extraction`.

- [ ] **Step 4: Add the listeners to the bootstrap**

In `crates/wwt-page/assets/bootstrap.js`, insert directly after `if (window.__wwt) return;`:
...
In `crates/wwt-page/src/extract.rs`, add near the other constants:
...
In `crates/wwt-page/src/extract.rs`, extend the raw deserialization struct:
...
`crates/wwt-page/src/lib.rs`:

```rust
pub use extract::{DIRTY_BINDING, Extraction, Page};
```

`crates/wwt/src/lib.rs:19`:
...
**Files:**
- Modify: `crates/wwt-page/src/extract.rs` (`navigate`, `wait_for_load`)
...
In `crates/wwt-page/src/extract.rs`, change the imports — `sleep` and `Instant` are no longer needed:
...
**Files:**
- Modify: `crates/wwt-page/src/extract.rs` (`scroll_by`, `scroll_to_top`, `scroll_to_end`)
- Create: `crates/wwt-page/tests/fixtures/tall.html`
- Modify: `crates/wwt-page/tests/extraction.rs` (tests)
...
`crates/wwt-page/tests/fixtures/tall.html`:
...
Add to `impl Page` in `crates/wwt-page/src/extract.rs`:
...
**Files:**
- Modify: `crates/wwt-page/src/extract.rs` (`go`, `back`, `forward`, `reload`)
- Modify: `crates/wwt-page/tests/extraction.rs` (test)
...
**Files:**
- Create: `crates/wwt-page/tests/fixtures/heavy.html`
- Create: `crates/wwt-page/tests/snapshots/simple.txt`
- Modify: `crates/wwt-page/tests/extraction.rs` (snapshot test, timing test)
- Modify: `crates/wwt-page/assets/bootstrap.js` (the rewrite)

No manifest changes: `wwt-frame`, which the snapshot helper needs, is already a dependency of `wwt-page`.
...
Add `wwt-frame` to `crates/wwt-page/Cargo.toml` dev-dependencies — it is already a normal dependency, so nothing changes; confirm rather than edit.

- [ ] **Step 2: Generate the snapshot from current behavior**

```bash
UPDATE_SNAPSHOTS=1 cargo test -p wwt-page --test extraction simple_page_matches_its_snapshot
cargo test -p wwt-page --test extraction simple_page_matches_its_snapshot
```

Expected: the second run PASSES against the file just written. Open `crates/wwt-page/tests/snapshots/simple.txt` and confirm it reads as the fixture's text laid out on a grid. If it is empty or garbled, stop — the snapshot must be right before it can guard anything.

- [ ] **Step 3: Add the heavy fixture and the measurement**

`crates/wwt-page/tests/fixtures/heavy.html`:
...
```bash
cargo test -p wwt-page --test extraction measure_extraction_of_a_heavy_page -- --nocapture
```
...
In `crates/wwt-page/assets/bootstrap.js`, replace the per-character grouping loop inside `extract` — everything from `// Group the node's characters into lines by their rounded top edge.` down to the closing brace of that `for` loop — with a call to a new helper, and add the helper above `function extract()`:
...
Expected: all `wwt-page` tests PASS, **including `simple_page_matches_its_snapshot` against the unmodified snapshot file**. If the snapshot differs, do not regenerate it — the rewrite changed behavior, and you must find out why first. A one-character shift is a real bug in the boundary search, not noise.
...
**Files:**
- Create: `crates/wwt/src/command.rs`
- Modify: `crates/wwt/src/lib.rs` (declare the module)

**Interfaces:**
- Consumes: nothing.
- Produces: `wwt::command::Command` (`Open(String)`, `Back`, `Forward`, `Reload`, `Quit`), deriving `Debug, Clone, PartialEq`; `command::parse(line: &str) -> Result<Command, String>` where `line` has had its leading `:` stripped by the caller; `command::normalize_url(raw: &str) -> Result<String, String>`.

- [ ] **Step 1: Write the failing tests**

Create `crates/wwt/src/command.rs` containing only the test module for now:
...
Add `pub mod command;` to `crates/wwt/src/lib.rs`, then:
...
Add above the test module in `crates/wwt/src/command.rs`:
...
```bash
git add crates/wwt
git commit -m "feat(wwt): parse the : command line"
```
...
**Files:**
- Create: `crates/wwt/src/chrome.rs`
- Create: `crates/wwt/src/keymap.rs`
- Modify: `crates/wwt/src/lib.rs` (declare the modules)

**Interfaces:**
- Consumes: `Command` from Task 11; `Frame`, `Style`, `CellPos`, `Viewport` from `wwt-frame`.
- Produces:
...
Create `crates/wwt/src/chrome.rs` with only its test module:
...
Add `pub mod chrome;` to `crates/wwt/src/lib.rs`, then:
...
Add above the test module in `crates/wwt/src/chrome.rs`:
...
Add `Frame` and `GridSize` to the test module's imports by ensuring `use super::*;` is joined by `use wwt_frame::{Frame, GridSize};` inside `mod tests`.

- [ ] **Step 4: Write the failing keymap tests**

Create `crates/wwt/src/keymap.rs` with only its test module:
...
Add `pub mod keymap;` to `crates/wwt/src/lib.rs`, then:
...
Add above the test module in `crates/wwt/src/keymap.rs`:
...
```bash
git add crates/wwt
git commit -m "feat(wwt): add the statusline, command line, and normal-mode keymap"
```
...
- Modify: `crates/wwt/Cargo.toml` (add `futures-util`)
- Create: `crates/wwt/src/core.rs`
- Modify: `crates/wwt/src/lib.rs` (declare the module)
...
Create `crates/wwt/src/core.rs` with only its test module. These test the parts that need no browser: the page viewport arithmetic, mode transitions, and composition.
...
Add `pub mod core;` to `crates/wwt/src/lib.rs`, then:
...
Add above the test module in `crates/wwt/src/core.rs`:
...
Add these imports to the test module so it compiles: `use wwt_frame::{CellSize, GridSize};` is already covered by `use super::*;`.
...
```bash
git add Cargo.toml Cargo.lock crates/wwt
git commit -m "feat(wwt): add the core event loop"
```
...
- Modify: `crates/wwt/src/lib.rs` (keep `render_url` for the smoke tests)
- Modify: `crates/wwt/tests/smoke.rs` (a modal-flow test)
- Modify: `README.md`
- Modify: `docs/superpowers/specs/2026-08-19-wwt-m2-design.md` (§11, the PTY row)

**Interfaces:**
- Consumes: `Core` from Task 13.
- Produces: the `wwt` binary.

- [ ] **Step 1: Rewrite `main`**

`crates/wwt/src/main.rs`:
...
Make `page_viewport` public in `crates/wwt/src/core.rs`:
...
`crates/wwt/src/lib.rs` gains the module declarations and keeps `render_url` unchanged from Task 6:
...
The spec's testing table names a PTY test. A `Core`-level test is strictly better here: it exercises the same modal flow deterministically, needs no new dependency, and cannot flake on timing. Add to `crates/wwt/tests/smoke.rs`:
...
Add `wwt-frame` and `crossterm` to `crates/wwt/Cargo.toml` — both are already dependencies, so confirm rather than edit.

- [ ] **Step 4: Correct the spec**

In `docs/superpowers/specs/2026-08-19-wwt-m2-design.md`, section 11, replace the `wwt` row's last sentence:

```
| `wwt` | Command parsing (`:open example.com` → `https://example.com`; unparseable input → error). Scroll arithmetic at each key. One test driving the modal flow through `Core`'s own types rather than a PTY: it covers the same `:`-to-command path deterministically, needs no new dependency, and cannot flake on process timing. |
```
...
```bash
git add crates/wwt README.md docs
git commit -m "feat(wwt): drive the browser from the core loop"
```
...
- `cargo test --workspace` is green, with 102 tests: 22 `wwt-frame`, 18 `wwt-term`, 8 `wwt-cdp`, 22 `wwt-page`, 32 `wwt`.
...
- `crates/wwt-page/tests/snapshots/simple.txt` passes unmodified through the Task 10 rewrite.
