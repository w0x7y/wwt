# wwt M1 — Walking Skeleton Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** `wwt <url>` launches headless Chromium, sizes its viewport to the terminal grid, extracts every text run's layout box, paints them into the cell grid, and quits on `q`.

**Architecture:** A Cargo workspace. `wwt-frame` owns all coordinate math and the cell grid with zero I/O, so the subtle logic is unit-testable without a browser. `wwt-cdp` is a hand-rolled CDP client over a websocket. `wwt-page` injects an extraction script and parses its output into `TextRun`s. `wwt-term` probes cell size and writes a `Frame` to a sink. The `wwt` binary wires them together behind one testable `render_url` function.
...
**Spec:** `docs/superpowers/specs/2026-08-19-wwt-design.md` — read spec sections 3, 4, and 5 before starting. This plan implements milestone M1 only.
...
- `wwt-frame` has **no I/O and no dependencies**. If a task tempts you to add one, the design is wrong — stop and ask.
- Chromium is located via the `WWT_CHROMIUM` environment variable, falling back to the first of `chromium`, `chromium-browser`, `google-chrome-stable` found on `PATH`. Never download anything.
...
- Create: `crates/wwt-frame/Cargo.toml`
- Create: `crates/wwt-frame/src/lib.rs`
- Create: `crates/wwt-frame/src/geom.rs`
- Test: `crates/wwt-frame/src/geom.rs` (inline `#[cfg(test)]` module)

**Interfaces:**
- Consumes: nothing.
- Produces: `wwt_frame::geom::{CellSize, GridSize, CellPos, CssPoint, CssRect, Viewport}`.
...
Create `crates/wwt-frame/src/geom.rs` containing only this test module for now:
...
Prepend to `crates/wwt-frame/src/geom.rs`, above the test module:
...
```bash
git add Cargo.toml Cargo.lock crates/wwt-frame
...
**Files:**
- Create: `crates/wwt-frame/src/cell.rs`
- Create: `crates/wwt-frame/src/run.rs`
- Create: `crates/wwt-frame/src/frame.rs`
- Modify: `crates/wwt-frame/src/lib.rs`
- Test: inline `#[cfg(test)]` module in `crates/wwt-frame/src/frame.rs`

**Interfaces:**
- Consumes: `Viewport`, `GridSize`, `CellPos`, `CssRect` from Task 1.
- Produces: `wwt_frame::cell::{Rgb, Style, Cell}`, `wwt_frame::run::TextRun`,
  `wwt_frame::frame::Frame` with `Frame::new(GridSize) -> Frame`,
...
Create `crates/wwt-frame/src/frame.rs` containing only this test module for now:
...
Update `crates/wwt-frame/src/lib.rs`:
...
Prepend to `crates/wwt-frame/src/frame.rs`, above the test module:
...
**Files:**
- Modify: `crates/wwt-frame/src/frame.rs` (the `paint_run` write loop)
- Test: inline `#[cfg(test)]` module in `crates/wwt-frame/src/frame.rs`
...
Append these tests inside the existing `mod tests` in `crates/wwt-frame/src/frame.rs`:
...
**Files:**
- Create: `crates/wwt-term/Cargo.toml`
- Create: `crates/wwt-term/src/lib.rs`
- Create: `crates/wwt-term/src/probe.rs`
- Modify: `Cargo.toml` (workspace members)
- Test: inline `#[cfg(test)]` module in `crates/wwt-term/src/probe.rs`

**Interfaces:**
- Consumes: `wwt_frame::{CellSize, GridSize}`.
- Produces: `wwt_term::probe::{WinSize, DEFAULT_CELL, cell_size_from, probe}`.
...
Add `"crates/wwt-term"` to `members` in the workspace `Cargo.toml`.

- [ ] **Step 2: Write the failing tests**

Create `crates/wwt-term/src/probe.rs` with only this test module for now:
...
Prepend to `crates/wwt-term/src/probe.rs`:
...
```bash
git add Cargo.toml Cargo.lock crates/wwt-term
...
**Files:**
- Create: `crates/wwt-term/src/render.rs`
- Modify: `crates/wwt-term/src/lib.rs`
- Test: inline `#[cfg(test)]` module in `crates/wwt-term/src/render.rs`

**Interfaces:**
- Consumes: `wwt_frame::{Frame, GridSize, CellPos, Style, Rgb}`.
- Produces: `wwt_term::render::render(frame: &Frame, out: &mut impl std::io::Write) -> std::io::Result<()>`.

Writing to any `Write` rather than to stdout is what makes this testable against a byte buffer.

- [ ] **Step 1: Write the failing tests**

Create `crates/wwt-term/src/render.rs` with only this test module for now:
...
Prepend to `crates/wwt-term/src/render.rs`:
...
Update `crates/wwt-term/src/lib.rs`:
...
**Files:**
- Create: `crates/wwt-cdp/Cargo.toml`
- Create: `crates/wwt-cdp/src/lib.rs`
- Create: `crates/wwt-cdp/src/launch.rs`
- Create: `crates/wwt-cdp/src/client.rs`
- Create: `crates/wwt-cdp/tests/browser.rs`
- Modify: `Cargo.toml` (workspace members)

**Interfaces:**
- Consumes: nothing from earlier tasks.
- Produces:
  `wwt_cdp::launch::{find_chromium, Chromium}` with
  `find_chromium() -> anyhow::Result<PathBuf>` and
  `Chromium::launch() -> anyhow::Result<Chromium>`, `Chromium::ws_url(&self) -> &str`;
  `wwt_cdp::client::Client` with
...
build, set `WWT_CHROMIUM` to its absolute path instead; the code honors it.
...
Add `"crates/wwt-cdp"` to `members` in the workspace `Cargo.toml`.
...
```bash
git add Cargo.toml Cargo.lock crates/wwt-cdp
...
**Files:**
- Create: `crates/wwt-page/Cargo.toml`
- Create: `crates/wwt-page/src/lib.rs`
- Create: `crates/wwt-page/src/color.rs`
- Create: `crates/wwt-page/src/extract.rs`
- Create: `crates/wwt-page/assets/extract.js`
- Create: `crates/wwt-page/tests/fixtures/simple.html`
- Create: `crates/wwt-page/tests/extraction.rs`
- Modify: `Cargo.toml` (workspace members)

**Interfaces:**
- Consumes: `wwt_cdp::{Chromium, Client}`, `wwt_frame::{Viewport, TextRun, Style, Rgb, CssRect}`.
- Produces:
  `wwt_page::color::parse_css_color(s: &str) -> Rgb`;
  `wwt_page::extract::Page` with
...
Add `"crates/wwt-page"` to `members` in the workspace `Cargo.toml`.

Create `crates/wwt-page/src/color.rs` with only this test module for now:
...
Prepend to `crates/wwt-page/src/color.rs`:
...
```bash
git add Cargo.toml Cargo.lock crates/wwt-page
...
**Files:**
- Create: `crates/wwt/Cargo.toml`
- Create: `crates/wwt/src/lib.rs`
- Create: `crates/wwt/src/main.rs`
- Create: `crates/wwt/tests/fixtures/skeleton.html`
- Create: `crates/wwt/tests/smoke.rs`
- Modify: `Cargo.toml` (workspace members)
- Create: `README.md`

**Interfaces:**
- Consumes: everything from Tasks 1 through 7.
- Produces: `wwt::render_url(url: &str, vp: Viewport) -> anyhow::Result<Frame>`.
...
wwt-cdp = { path = "../wwt-cdp" }
wwt-frame = { path = "../wwt-frame" }
wwt-page = { path = "../wwt-page" }
wwt-term = { path = "../wwt-term" }
anyhow.workspace = true
crossterm.workspace = true
tokio.workspace = true
...
Add `"crates/wwt"` to `members` in the workspace `Cargo.toml`.
...
Expected: FAIL to compile — `wwt::render_url` not found.
...
async fn main() -> Result<()> {
    ...
    let frame = wwt::render_url(&url, vp).await?;
...
fn run(frame: &wwt_frame::Frame) -> Result<()> {
    let mut out = stdout();
    wwt_term::render(frame, &mut out)?;
    out.flush()?;
...
```bash
git add Cargo.toml Cargo.lock crates/wwt README.md
...
- `cargo test --workspace` is green, with 47 tests: 17 in `wwt-frame`, 13 in
  `wwt-term`, 3 in `wwt-cdp`, 12 in `wwt-page`, 2 in `wwt`.
- `cargo clippy --workspace --all-targets -- -D warnings` is clean.
- `cargo run -p wwt -- https://example.com` shows the page's text laid out
