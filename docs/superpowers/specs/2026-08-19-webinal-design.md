...
A terminal web browser in Rust, intended as a genuine daily driver rather than a
text-mode curiosity. It drives a real headless Chromium over the Chrome DevTools
Protocol (CDP) and renders pages into the terminal grid two ways: a crisp
text-mode reconstruction by default, and true pixel rendering on demand.
