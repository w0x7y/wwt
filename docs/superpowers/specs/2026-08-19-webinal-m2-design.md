# Webinal M2 — Navigation and Reading

**Date:** 2026-08-19
**Status:** Approved, pre-implementation
**Parent spec:** `2026-08-19-webinal-design.md` — sections 5, 6, and 8 govern here.
...
At the end of M2, `webinal https://example.com` is a usable read-only browser. It
...
| Crate | Change |
|---|---|
| `wb-frame` | `Frame::paint_text`, `Style.reverse`. Still no I/O, still no dependencies. |
| `wb-cdp` | Event subscription. `Page` ownership moves from borrow to `Arc`. |
| `wb-page` | Installed script, navigation, scroll, history, event-driven load. |
| `wb-term` | `Renderer`: a diffing renderer holding the last presented frame. |
| `webinal` | `core.rs` (the loop and all state), `chrome.rs` (statusline and command line). |

`wm-ui` is **not** created in M2. The parent spec's component table assigns chrome to
...
`wm-ui` when hint overlays and the modal state machine arrive, and moves M2's chrome
into it.

## 3. The event pump

`wb-cdp`'s `read_loop` currently drops every message without an `id`. Those are
...
`Page<'a>` borrows its `Client`, which is why `wb-page`'s test harness holds the two
...
navigation, per parent spec section 4. It defines `window.__webinal.extract()` and
installs the dirty-signal listeners.

**Extraction** becomes `Runtime.evaluate("__webinal.extract()")` — one round trip
returning one flat array, unchanged in shape from M1.

### The dirty signal

`Runtime.addBinding` registers `__webinal_dirty` on the session. The bootstrap calls
it from:
...
`wb-term::render` repaints all 8,640 cells every time. M2 replaces it with a
...
| Crate | Tests |
|---|---|
| `wb-frame` | `paint_text` placement and clipping at the grid edge; `reverse` styling. |
